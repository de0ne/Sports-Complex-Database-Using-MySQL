-- create a database and name it sports_bookings
CREATE DATABASE sports_bookings;
USE sports_bookings;

-- use the create table statements to create four tables
CREATE TABLE members (id VARCHAR(255) PRIMARY KEY, password VARCHAR(255) NOT NULL,
email VARCHAR(255) NOT NULL, member_since TIMESTAMP NOT NULL DEFAULT NOW(),
payment_due DECIMAL(6,2) NOT NULL DEFAULT 0);

CREATE TABLE pending_terminations (id VARCHAR(255) PRIMARY KEY, email VARCHAR(255) NOT NULL,
request_date TIMESTAMP NOT NULL DEFAULT NOW(), payment_due DECIMAL(6,2) NOT NULL DEFAULT 0);

CREATE TABLE rooms (id VARCHAR(255) PRIMARY KEY, room_type VARCHAR(255) NOT NULL, 
price DECIMAL(6,2) NOT NULL);

CREATE TABLE bookings (id INT AUTO_INCREMENT PRIMARY KEY, room_id VARCHAR(255),
booked_date DATE NOT NULL, booked_time TIME NOT NULL, member_id VARCHAR(255), 
...
-- add values to the tables created
...
INSERT INTO bookings (id, room_id, booked_date, booked_time, member_id, 
datetime_of_booking, payment_status) VALUES
...
datetime_of_booking, price, payment_status FROM bookings JOIN rooms
ON bookings.room_id = rooms.id
ORDER BY 1;

SELECT * FROM member_bookings;

-- nine procedures, one trigger and one stored function
DELIMITER $$
...
CREATE PROCEDURE make_booking (IN p_room_id VARCHAR(255), IN p_booked_date DATE, p_booked_time TIME, p_member_id VARCHAR(255))
BEGIN
...
BEGIN
	SELECT * FROM rooms WHERE id NOT IN (SELECT room_id FROM bookings WHERE booked_date = p_booked_date 
		AND booked_time = p_booked_time AND payment_status != 'Cancelled') AND
...
BEGIN
    ...
    SELECT member_id, booked_date, price, payment_status INTO v_member_id, v_booked_date, 
		v_price, v_payment_status FROM member_bookings WHERE id = p_booking_id;
	SELECT payment_due INTO v_payment_due FROM members WHERE id = v_member_id;
	IF CURDATE() >= v_booked_date THEN
		...
		ELSE 
            ...
            IF v_cancellation >= 3 THEN SET v_payment_due = v_payment_due + 10;
			END IF;
            UPDATE members
				SET payment_due = v_payment_due WHERE id = v_member_id;
...
DELIMITER ;
-- we've come to the end of the project, thanks for checking it out.
-- for more detailed write up, please check my medium article, check the read me file for this project to get the medium link.